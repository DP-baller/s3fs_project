...
// This function encrypts or decrypts the content of inputFile using the RC4 cipher.
void CryptRC4(int inputFile)
{
    unsigned char rc4_buffer[4096]; // Buffer to store the content read from the inputFile

    const char *user_key = "mykey"; // User-defined key for encryption/decryption

    unsigned char derived_key[EVP_MAX_KEY_LENGTH]; // Derived key generated using the user_key and a hash function

    RC4_KEY key; // RC4 key structure to store the derived key

    // Generate the derived key using the user_key, EVP_rc4() cipher, and EVP_sha256() hash function
    EVP_BytesToKey(EVP_rc4(), EVP_sha256(), NULL, (const unsigned char *)user_key, (int)strlen(user_key), 1, derived_key, NULL);

    // Set the derived key in the RC4_KEY structure
    RC4_set_key(&key, (int)strlen((const char *)derived_key), derived_key);

    // Position the file offset to the current location
    lseek(inputFile, 0, SEEK_CUR);

    // Read from the inputFile into the rc4_buffer
    ssize_t actual_size = read(inputFile, rc4_buffer, sizeof(rc4_buffer));

    // Continue until there's no more data to read from inputFile
    while (actual_size > 0)
    {
        // Encrypt or decrypt the content in the rc4_buffer using the RC4 cipher and the derived key
        RC4(&key, (unsigned long)actual_size, (const unsigned char *)rc4_buffer, rc4_buffer);

        // Move the file offset back by actual_size, so the encrypted/decrypted content will overwrite the original content
        lseek(inputFile, -actual_size, SEEK_CUR);

        // Write the encrypted/decrypted content back to the inputFile
        write(inputFile, rc4_buffer, actual_size);

        // Read the next chunk of data from the inputFile into the rc4_buffer
        actual_size = read(inputFile, rc4_buffer, sizeof(rc4_buffer));
    }
}
